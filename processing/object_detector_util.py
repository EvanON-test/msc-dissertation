...
#Realtime version - removed all the extra bb stuff and went back to original
...
#TODO:revisit this at a time when time is less precious

# def process_trt(savepoint):
#     cropped_frames = []
#
#     fixed_box_size = np.asarray([539, 561])
#
...
#     # #     [1, 640, 640, 3])
#     # interpreter.allocate_tensors()
#     with open("../od_pytorch/best.engine", "rb") as f:
...
#
#     cuda_input = cuda.mem_alloc(trt.volume(input_shape) * np.float32().itemsize)
#     cuda_output = cuda.mem_alloc(trt.volume(output_shape) * np.float32().itemsize)
#     stream = cuda.Stream()
#
#     # # Get input and output tensors.
#     # input_details = interpreter.get_input_details()
